...
// Images de fallback Unsplash — utilisées quand l'hôtel n'a pas encore de galerie en BDD
...
export default function DetailsPage() {
  ...
  // Slides : priorité aux images de l'hôtel, sinon fallback Unsplash
  const hotelImages = hotel.images?.length > 0
      ? hotel.images
      : hotel.imageUrl
          ? [hotel.imageUrl, ...FALLBACK_IMAGES.slice(0, 4)]
          : FALLBACK_IMAGES;

  const slides = hotelImages.map((img) =>
  ...
  return (
      <div className="min-h-screen bg-[#F8FAFC]">
          ...
          <div className="relative h-[340px] md:h-[500px] w-full overflow-hidden bg-gray-200">
            ...
            {slides.length > 1 && (
                <>
                  <button
                      onClick={prevSlide}
                      className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-2.5 shadow-lg transition"
                  ...
                  <button
                      onClick={nextSlide}
                      className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white rounded-full p-2.5 shadow-lg transition"
                    ...
                    {slides.map((_, i) => (
                        <button
                            ...
                            className={`h-1.5 rounded-full transition-all ${
                                i === currentSlide ? "bg-white w-8" : "bg-white/60 w-1.5 hover:bg-white/80"
                            }`}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 pb-16 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div>
              <div className="flex flex-wrap gap-2 mb-3">
                {hotel.categorie && (
                  ...
                  {hotel.categorie === 5 ? "Luxe" : hotel.categorie === 4 ? "Premium" : hotel.categorie === 3 ? "Confort" : "Économique"}
                </span>
                )}
              ...
              <div className="flex items-center gap-4 flex-wrap">
                ...
                {/* Classement + Avis — même logique que CardHotel */}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {hotel.equipements.map((eq) => (
                        <div
                            key={eq}
                            className="flex items-center gap-3 border border-gray-100 rounded-xl p-3.5 bg-white hover:border-[#0EA5E9]/30 hover:shadow-sm transition-all"
                        >
            ...
            <div>
              ...
              <p className="text-gray-600 leading-relaxed text-sm">
                {hotel.description && hotel.description.trim()
                    ? hotel.description
                    : `À propos de l'hôtel — Cet établissement vous accueille dans un cadre élégant au cœur de ${hotel.ville || "la ville"}. Description détaillée à venir.`}
              </p>
            </div>
          ...
          {/* Colonne droite (1/3) — CONTENEUR UNIQUE sticky pour que tout scrolle ensemble */}
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-6 space-y-4">

              {/* Carte prix + CTA */}
              <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                <p className="text-xs text-gray-500 mb-1 uppercase tracking-wider">À partir de</p>
                ...
                <div className="space-y-2.5 mb-5 text-sm text-gray-600">
                  {hotel.noteMoyenne && (
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-xs">Noté {hotel.noteMoyenne}/5 (350 avis)</span>
                      </div>
                  )}
                  <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-xs">Établissement recommandé</span>
                  </div>
                </div>

                <Link
                    to="/Connexion"
                    className="block w-full bg-[#0EA5E9] hover:bg-[#0284C7] text-white text-center font-semibold py-3 rounded-xl transition-colors mb-2 text-sm"
                >
              ...
              <div className="bg-[#F0F9FF] border border-[#BAE6FD] rounded-2xl p-4 flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-[#0369A1] text-sm">Offre spéciale</p>
                  <p className="text-[#0EA5E9] text-xs mt-0.5">
